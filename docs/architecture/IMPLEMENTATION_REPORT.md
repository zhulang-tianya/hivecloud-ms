...
| 模块 | Redis | MySQL | Nacos | Caffeine | OkHttp | Gossip | 部署优先级 |
|------|-------|-------|-------|----------|--------|--------|-----------|
| hivecloud-gateway | 必须 | 可选 | 必须 | 必须 | 必须 | 必须 | P0 - 必选 |
| hivecloud-module-system | 必须 | 必须 | 必须 | 必须 | 可选 | 必须 | P0 - 必选 |
| hivecloud-module-payment | 必须 | 必须 | 必须 | 必须 | 必须 | 必须 | P1 - 可选 |
| hivecloud-module-claim | 必须 | 必须 | 必须 | 必须 | 必须 | 必须 | P1 - 可选 |
| hivecloud-module-activity | 必须 | 必须 | 必须 | 必须 | 可选 | 必须 | P2 - 可选 |
| hivecloud-module-notify | 必须 | 必须 | 必须 | 必须 | 可选 | 必须 | P2 - 可选 |
| hivecloud-plugin-file | 必须 | 必须 | 必须 | 必须 | 可选 | 必须 | 插件 - 按需 |

---

## 5.3 部署模式

### 5.3.1 推荐部署方案：核心 + 按需扩展
...
│                    必选服务清单                          │
├─────────────────────────────────────────────────────────┤
│ 1. hivecloud-gateway（统一业务网关）                     │
│    - 公网流量统一入口                                    │
│    - 负责鉴权、限流、路由转发                            │
│    - 独立部署，支持水平扩展                              │
│    - 依赖：Redis（限流）、Nacos（配置）                  │
│                                                         │
│ 2. hivecloud-module-system（系统管理模块）               │
│    - 用户、角色、权限、菜单管理                          │
│    - 提供登录认证和授权能力                              │
│    - 独立部署                                            │
│    - 依赖：MySQL（用户数据）、Redis（Token）             │
└─────────────────────────────────────────────────────────┘
```
...
│                    可选服务清单                          │
├─────────────────────────────────────────────────────────┤
│ 3. hivecloud-module-payment（支付模块）                  │
│    - 订单支付、退款、对账                                │
│    - 核心交易链路，按需独立部署                          │
│    - 依赖：MySQL（订单数据）、Redis（缓存）              │
│                                                         │
│ 4. hivecloud-module-claim（理赔模块）                    │
│    - 理赔申请、审核、赔付                                │
│    - 核心业务链路，按需独立部署                          │
│    - 依赖：MySQL（理赔数据）、Redis（缓存）              │
└─────────────────────────────────────────────────────────┘
```
...
│                    扩展服务清单                          │
├─────────────────────────────────────────────────────────┤
│ 5. hivecloud-module-activity（活动模块）                 │
│    - 活动创建、参与、奖励发放                            │
│    - 营销活动相关，按需部署                              │
│                                                         │
│ 6. hivecloud-module-notify（通知模块）                   │
│    - 短信、邮件、站内信                                  │
│    - 消息通知能力，按需部署                              │
└─────────────────────────────────────────────────────────┘
```
...
└────────────────────┬────────────────────────────────────────┘
        ...
        │                         │
   ┌────▼────┐              ┌────▼────┐
   │ Gateway │              │ Gateway │  (集群部署，Redis 共享限流)
   │  Node1  │              │  Node2  │
   └────┬────┘              └────┬────┘
        │                         │
        └────────────┬────────────┘
        ...
        │            │            │               │          │
   ┌────▼────┐  ┌───▼────┐  ┌───▼────┐  ┌──────▼─────┐  ┌──▼──┐
   │ System  │  │Payment │  │ Claim  │  │  Activity  │  │Notify│
   │  必选   │  │  可选  │  │  可选  │  │    可选    │  │ 可选 │
   └────┬────┘  └───┬────┘  └───┬────┘  └──────┬─────┘  └──┬──┘
        │          │          │               │          │
        └──────────┴──────────┴───────────────┴──────────┘
...
| 场景 | 推荐方案 | 服务清单 | 说明 |
|------|---------|---------|------|
| 开发环境 | 单体部署 | 所有模块合并 | 快速开发调试，单数据库 |
| 测试环境 | 核心分离 | gateway + system + 其他合并 | 模拟生产，降低成本 |
| 生产环境（初期） | 核心 + 按需 | gateway + system + payment/claim | 核心业务独立，其他按需 |
| 生产环境（成熟） | 完全微服务 | 所有服务独立部署 | 高可用，弹性伸缩 |
